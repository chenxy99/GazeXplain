...
class ResNetCOCO(nn.Module):
    def __init__(self):
        ...
        self.resnet = maskrcnn_resnet50_fpn(weights=MaskRCNN_ResNet50_FPN_Weights.COCO_V1).backbone.body
...
class IntegrationModule(nn.Module):
    def __init__(self, channel_size, im_h, im_w, dropout):
        ...
        self.vision_semantic_feature = nn.Linear(channel_size, channel_size, bias=True)

        self.linear_layer = nn.Linear(2 * channel_size, channel_size, bias=True)

        self.init_weights()

    def forward(self, features, visual_att):
        visual_feature = features.permute(1, 0, 2)
        visual_att = visual_att.unsqueeze(-1)
        x = visual_feature * visual_att

        u_s = F.relu(self.vision_spatial_feature(x.sum(-1))).unsqueeze(-1)
        u_c = F.relu(self.vision_semantic_feature(x.sum(-2))).unsqueeze(-2)
        R = u_s * u_c
...
class Transformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                encoder_dropout, activation, normalize_before)
        encoder_norm = nn.LayerNorm(d_model)
        encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)
        input_proj = nn.Linear(img_hidden_dim, d_model)
        img_transform = nn.Linear(d_model, d_model)
        text_transform = nn.Linear(lm_dmodel, d_model)
        self.encoder = TransformerEncoderWrapper(encoder, input_proj, img_transform, text_transform, encoder_dropout)

        decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                decoder_dropout, activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                     return_intermediate=return_intermediate_dec)

        dropout = nn.Dropout(decoder_dropout)
        self.decoder = TransformerDecoderWrapper(d_model, activation, decoder, dropout)

        self.integration_module = IntegrationModule(hidden_dim, im_h, im_w, encoder_dropout)
    ...
    def forward(self, src: Tensor, tgt: Tensor, task: Tensor, src_mask: Optional[Tensor] = None,
        ...
        return memory, memory_task, output


class TransformerEncoderWrapper(nn.Module):

    def __init__(self, encoder, input_proj, img_transform, text_transform, dropout):
        super().__init__()
...
class TransformerDecoderWrapper(nn.Module):
    def __init__(self, d_model, activation, decoder, dropout):
        super().__init__()
