...
import numpy as np
import math
from lib.evaluation.pycocoevalcap.ciderR import utils
...
class CiderScorer(object):
    ...
    def __init__(self, refs, test=None, n=4, sigma=6.0, doc_frequency=None, ref_len=None):
        ...
        for k in refs.keys():
            ...
            if test is not None:
                self.test.append(test[k][0])
            else:
                self.test.append(None)
            if test is not None:
    ...
    def compute_cider(self):
        ...
        # compute log reference length
        # I prefer to use the overall training set when applying SCST, since in SCST the length of crefs is small
        # self.ref_len = np.log(float(len(self.crefs)))
