...
test_json_data = os.path.join(fixation_root, 'AiR_fixations_test.json')
...
for scanpath in fixations:
    key = '{}-{}-{}'.format(scanpath['split'], scanpath['question_id'], scanpath['image_id'][:-4])
    raw_height = scanpath["height"]
    raw_width = scanpath["width"]
